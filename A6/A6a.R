...
data <- read.csv('GOOGL.csv', colClasses = col_classes)
...
boxplot(data$Close, main="Boxplot for Close Price", ylab="Price")

# Plot the data
ggplot(data, aes(x = Date, y = Close)) +
  geom_line() +
  labs(title = "Google Stock closing Price Over Time", x = "Date", y = "Price")
...
autoplot(decomp_additive) +
  ggtitle("Additive Decomposition of GOOGLE Stock Price") +
  theme_minimal()
...
autoplot(hw_forecast) +
  ggtitle("Holt-Winters Forecast for GOOGLE Stock Price") +
  theme_minimal()
...
autoplot(daily_forecast) +
  ggtitle("Daily Forecast for Google Stock Price") +
  theme_minimal()
...
autoplot(monthly_forecast) +
  ggtitle("Monthly ARIMA Forecast for Google Stock Price") +
  theme_minimal()
...
Googl_data <- Googl_data %>% drop_na()

# Split the data into training and testing sets
set.seed(123)
train_index <- createDataPartition(googl_data$Close, p = 0.8, list = FALSE)
train_data <- googl_data[train_index, ]
test_data <- googl_data[-train_index, ]
